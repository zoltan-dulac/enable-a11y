...
const enableSlider = function (
  ...
  this.init = function () {
    ...
    this.$template = document.getElementById("enable-slider__handle--template");
  ...
  this.positionRangeDiv = () => {
    ...
    if (!this.vert) {
      ...
      size =
        Math.round(
          ((this.val2 - this.min) / (this.max - this.min)) * offsetWidth
        ) - pos;
      ...
      size =
        Math.round(
          ((this.val2 - this.min) / (this.max - this.min)) * offsetHeight
        ) - pos;
   ...
   * @returns { boolean } true if propagating; false if consuming event
   */
  this.handleFocus = function ($handle, evt) {
    $handle.setAttribute(
      "src",
   ...
   * @returns { boolean } true if propagating; false if consuming event
   */
  this.handleBlur = function ($handle, evt) {
    $handle.setAttribute(
      "src",
  ...
  this.handlePointerMove = function ($handle, evt) {
    ...
    if (!this.vert) {
      ...
      // Calculate the new slider value based on the horizontal pixel position of the pointer
      newVal =
        Math.round(
          ((evt.pageX - offsetLeft) / offsetWidth) * (this.max - this.min)
        ) + this.min;
    } else {
      // vertical slider
      ...
      newVal =
        Math.round(
          ((evt.pageY - offsetTop) / offsetHeight) * (this.max - this.min)
        ) + this.min;
    }
