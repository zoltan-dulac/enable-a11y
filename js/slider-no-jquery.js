...
slider.prototype.createRangeDiv = function () {
  ...
  var range =
    '<div id="' + id + '" class="' + this.className + '__slider-range"></div>';
...
slider.prototype.positionRangeDiv = function () {
  ...
  if (this.vert == false) {
    ...
    pos = Math.round(
      ((this.val1 - this.min) / (this.max - this.min)) * this.width
    );

    // calculate the new range width
    size =
      Math.round(
        ((this.val2 - this.min) / (this.max - this.min)) * this.width
      ) - pos;
    ...
    // calculate the range start position
    pos = Math.round(
      ((this.val1 - this.min) / (this.max - this.min)) * this.height
    );

    // calculate the new range width
    size =
      Math.round(
        ((this.val2 - this.min) / (this.max - this.min)) * this.height
      ) - pos;
...
slider.prototype.handleFocus = function ($handle, evt) {
  $handle.setAttribute(
    "src",
    "images/slider_" + (this.vert == true ? "v" : "h") + "-focus.png"
  );
...
slider.prototype.handleBlur = function ($handle, evt) {
  $handle.setAttribute(
    "src",
    "images/slider_" + (this.vert == true ? "v" : "h") + ".png"
  );
...
slider.prototype.handleMouseMove = function ($handle, evt) {
  ...
  if (this.vert == false) {
    ...
    newVal =
      Math.round(
        ((evt.pageX - this.left) / this.width) * (this.max - this.min)
      ) + this.min;
  } else {
    // vertical slider
